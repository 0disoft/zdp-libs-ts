...
export function calculateSecurityCostBreakEven(
  ...
  const base = parseNamedUnitDecimal(
    input.monthlyBaseFixedCost,
    'monthly_base_fixed_cost'
  );
  if (!base.ok) {
    return base;
  }
  const security = parseNamedUnitDecimal(
    input.monthlySecurityCost,
    'monthly_security_cost'
  );
  if (!security.ok) {
    return security;
  }
  ...
  const variable = parseNamedUnitDecimal(
  ...
  const total = addRationals(
    decimalToRational(base.decimal),
    decimalToRational(security.decimal)
  );
  const contribution = subtractRationals(
    decimalToRational(price.decimal),
    decimalToRational(variable.decimal)
  );
