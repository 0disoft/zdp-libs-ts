...
export function calculateUnattendedLaborSavings(
  ...
  const current = parseNamedUnitDecimal(
    input.currentMonthlyLaborCost,
    'current_monthly_labor_cost'
  );
  if (!current.ok) {
    return current;
  }
  const unattended = parseNamedUnitDecimal(
    input.unattendedMonthlyLaborCost,
    'unattended_monthly_labor_cost'
  );
  if (!unattended.ok) {
    return unattended;
  }
  const system = parseNamedUnitDecimal(
    input.additionalMonthlySystemCost,
    'additional_monthly_system_cost'
  );
  if (!system.ok) {
    return system;
  }
  if (
    [current, unattended, system].some(
  ...
  const gross = subtractRationals(
    decimalToRational(current.decimal),
    decimalToRational(unattended.decimal)
  );
