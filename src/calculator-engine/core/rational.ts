...
export function decimalDigitCount(value: bigint): number {
  const absolute = value < 0n ? -value : value;
  return absolute.toString().length;
}

export function formatRational(value: Rational, decimalPlaces: number): string {
...
export function percentOfRational(
  numerator: Rational,
