import { CALCULATORS, CALCULATOR_IDS, CALCULATOR_REQUIRED_ENGINE_VERSION } from '../calculator-engine/catalog.generated.js';
...
function validateGeneratedCalculatorCatalog(apiContractsInput, diagnostics) {
    const catalog = apiContractsInput.calculatorCatalog;
    const conformance = apiContractsInput.calculatorConformance;
    const reviewedDefinitions = catalog.definitions.filter((definition) => definition.lifecycleStatus === 'reviewed');
    const reviewedIds = reviewedDefinitions.flatMap((definition) => definition.id === null ? [] : [definition.id]);
    if (!haveSameOrderedValues(reviewedIds, CALCULATOR_IDS)) {
        diagnostics.push({
            ...
            path: 'definitions',
            message: 'Reviewed calculator ids must match the generated calculator registry. Run `bun run calculator-catalog:generate`.'
        });
    }
    for (const calculatorId of CALCULATOR_IDS) {
        ...
        if (definition.lifecycleStatus !== expected.lifecycleStatus ||
            definition.contractVersion !== expected.contractVersion ||
            !haveSameOrderedValues(definition.compatibleEngineVersions, expected.compatibleEngineVersions) ||
...
function haveSameOrderedValues(actual, expected) {
